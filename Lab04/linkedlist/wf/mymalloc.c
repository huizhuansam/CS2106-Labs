...
void *mymalloc(size_t size) {
  ...
  TNode *trav = _memlist;
  TNode *worst_node = NULL;

  while (trav) {
    TData *trav_data = trav -> pdata;
    if ((trav_data -> status) == HOLE && (trav_data -> size) >= size) {
      if (worst_node == NULL || (trav_data -> size) >= (worst_node -> pdata -> size)) {
        worst_node = trav;
      }
    }
    trav = trav -> next;
  ...
  TData *worst_data = worst_node -> pdata;
  TData *data = (TData*) malloc(sizeof(TData));
  data -> size = size;
  data -> status = OCCUPIED;
  TNode *node = make_node(worst_node -> key, data);
  
  if ((worst_data -> size) > size) {
    TData *hole_data = (TData*) malloc(sizeof(TData));
    hole_data -> size = (worst_data -> size) - size;
    hole_data -> status = HOLE;
    unsigned int next_hole_index = (worst_node -> key) + size;
    TNode *hole_node = make_node(next_hole_index, hole_data);
    insert_node(&_memlist, hole_node, ASCENDING);
  }

  free(worst_node -> pdata);
  worst_node -> pdata = NULL;
  delete_node(&_memlist, worst_node);
